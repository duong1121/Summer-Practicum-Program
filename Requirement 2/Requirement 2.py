...
def scrape_google_images():
    ...
    LIST_img=[]
    LIST_name=[]
    url = ''  #Change url to where we want to download image
    headers = {
    ...
    scrape_images(url, 'true', LIST_img, LIST_name)

    # Scrape and download AI-generated images
    scrape_images(url, 'AI-generated', LIST_img, LIST_name)
    ...
    for i in LIST_img:
        if(count<1000):
            LIST_img[count].save(os.path.join(folder1_path, LIST_name[count]))
        else:
            LIST_img[count].save(os.path.join(folder2_path, LIST_name[count]))
        count += 1
